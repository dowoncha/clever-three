...
export const tEasings = {
  ...
  InSine: (t) => { return 1 - Math.cos(t * Math.PI / 2) },
  OutSine: (t) => { return Math.sin(t * Math.PI / 2) },
  InOutSine: (t) => { return 0.5 * (1 - Math.cos(Math.PI * t)) },
  InBounce: (t) => { return 1 - tEasings.OutBounce(1 - t) },
  OutBounce: (t) => {
  ...
  InOutBounce: (t) => {
    if (t < 0.5) {
      return tEasings.InBounce(t * 2) * 0.5
    }
    return tEasings.OutBounce(t * 2 - 1) * 0.5 + 1 * 0.5
  },
  ...
  InOutElastic: (t, amplitude, period) => {
    ...
    return (
      (
        amplitude * Math.pow(2, 10 * t) *
        Math.sin((t - offset) *
        (Math.PI * 2) / period)
      ) / -2
    )
  },
  InExpo: (t) => { return Math.pow(2, 10 * (t - 1)) },
  OutExpo: (t) => { return -Math.pow(2, -10 * t) + 1 },
  InOutExpo: (t) => {
    if (t === 0) { return 0 }
    if (t === 1) { return 1 }
