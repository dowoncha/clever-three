...
export default {
  ...
  vec3 findClosest (in vec3 ref, in int mode) {
    ...
    // CGA Mode 4 palette #1 with both intensities (low and high). The good old cyan-magenta "7-color" palette
      ...
      try (vec3 ( 30.0,  30.0,  30.0)); //  0 - black
    ...
    else if ( mode == 7 ) {             // 5 COLORS (GREENISH - GAMEBOY)
    ...
    else if ( mode == 8 ) {             // 5 COLORS (PURPLEISH)
    ...
    else if ( mode == 9 ) {             // 11 COLORS (GRAYSCALE)
  ...
  vec3 getColorFX ( in int fx, in vec2 pos, in vec3 col ) {
    const float numberOfEffects = 12.0;
    fx = int( fract( float( fx ) * 1.61456 ) * numberOfEffects );
    int temp = int( float( fx ) / ( numberOfEffects - 1.0 ) );
    fx -= temp * int( numberOfEffects - 1.0 );
    vec3 color = vec3( 0.0 );
  ...
  vec3 getScreenOutput( in vec2 uv, in vec3 col, in float effectLength ) {
  ...
  void main(void) {
    ...
    vec3 color = vec3(0.0);
    if (mode > 0.0) {
      color = dither(tex.rgb, uv, int(floor(mode)));
    } else {
      color = getScreenOutput(uv, tex.rgb, effectDuration);
    }
