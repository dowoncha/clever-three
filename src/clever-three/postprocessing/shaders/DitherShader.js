...
export default {
  ...
  float dither ( in mat4 m, in ivec2 p ) {
    if ( p.y == 0 ) {
  ...
  void main () {
    ivec2 bp = ivec2( mod( gl_FragCoord.xy, res ) );
    vec3 c = texture2D( tDiffuse, vUv ).xyz;
    c -= 1.0 / 255.0;
    vec3 d = vec3(0.0);
    if ( bp.x <= 3 && bp.y <= 3 ) {
      d.r = float( c.r > dither( bayertl, bp ) );
      d.g = float( c.g > dither( bayertl, bp ) );
      d.b = float( c.b > dither( bayertl, bp ) );
    } else if ( bp.x > 3 && bp.y <= 3 ) {
      d.r = float( c.r > dither( bayertr, bp - ivec2(4,0) ) );
      d.g = float( c.g > dither( bayertr, bp - ivec2(4,0) ) );
      d.b = float( c.b > dither( bayertr, bp - ivec2(4,0) ) );
    } else if ( bp.x <= 3 && bp.y > 3 ) {
      d.r = float( c.r > dither( bayerbl, bp - ivec2(0,4) ) );
      d.g = float( c.g > dither( bayerbl, bp - ivec2(0,4) ) );
      d.b = float( c.b > dither( bayerbl, bp - ivec2(0,4) ) );
    } else if ( bp.x > 3 && bp.y > 3 ) {
      d.r = float( c.r > dither( bayerbr, bp - ivec2(4,4) ) );
      d.g = float( c.g > dither( bayerbr, bp - ivec2(4,4) ) );
      d.b = float( c.b > dither( bayerbr, bp - ivec2(4,4) ) );
  }
  vec3 o = texture2D( tDiffuse, vUv ).xyz;
  gl_FragColor = vec4( mix( o, d, amount ), 1.0 );
  }`
}
